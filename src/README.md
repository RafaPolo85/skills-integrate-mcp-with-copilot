...
Teacher credentials are stored in `teachers.json`. The default local development account is
`teacher` / `mergington-admin`. Authentication tokens are kept in memory and expire when the
server restarts.
